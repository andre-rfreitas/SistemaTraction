...
public class SeparationItem : BaseEntity
{
    ...
    public static SeparationItem Create(
        Guid separationListId, string sku, string color, string size, int quantity, int sortOrder)
    {
    ...
    public void Update(string sku, string color, string size, int quantity)
    {
        if (string.IsNullOrWhiteSpace(color))
            throw new DomainException("Cor é obrigatória.");
