...
BEGIN
    ALTER TABLE [SeparationItems] DROP CONSTRAINT [FK_SeparationItems_SeparationLists_SeparationListId1];
END;
...
BEGIN
    DROP INDEX [IX_SeparationItems_SeparationListId1] ON [SeparationItems];
END;
...
BEGIN
    ...
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[SeparationItems]') AND [c].[name] = N'SeparationListId1');
    IF @var IS NOT NULL EXEC(N'ALTER TABLE [SeparationItems] DROP CONSTRAINT [' + @var + '];');
    ALTER TABLE [SeparationItems] DROP COLUMN [SeparationListId1];
END;
